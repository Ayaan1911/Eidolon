...
async function failure(res) {
  ...
  // FastAPI's own validation errors (422) put an ARRAY of {msg, loc, ...} objects in
  // `detail`, not a string. Passed straight to `new Error()`, that array stringifies via
  // each object's default toString - "[object Object]" - so it's joined into text here.
  const detail = Array.isArray(data.detail)
    ? data.detail.map((d) => d.msg || JSON.stringify(d)).join("; ")
    : data.detail
  return Object.assign(new Error(detail || `Request failed (${res.status})`), { status: res.status })
}
...
export async function deleteTrap(trapId) {
  const headers = adminHeaders()
  if (!headers) throw new Error("Deleting traps is admin-only in this public demo.")
  const res = await fetch(`${API_BASE}/api/traps/${trapId}`, { method: "DELETE", headers })
  if (!res.ok) throw await failure(res)
}
